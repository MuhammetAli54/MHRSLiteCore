...
namespace MHRSLite_UI.Components
{
    public class PatientAppointmentsViewComponent : ViewComponent
    {
        ...
        public IViewComponentResult Invoke(int pageNumberPast = 1, int pageNumberFuture = 1)
        {
            ...
            //    data.UpcomingAppointments = _unitOfWork.AppointmentRepository.GetAll(x => x.PatientId == HttpContext.User.Identity.Name
            //  &&
            ...
            //    data.PastAppointments = _unitOfWork.AppointmentRepository.GetAll(x => x.PatientId == HttpContext.User.Identity.Name
            //  &&
            //  x.AppointmentDate <= today
            // &&
            // (x.AppointmentDate == today
